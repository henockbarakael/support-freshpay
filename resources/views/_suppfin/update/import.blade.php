                    ...
                    <tbody class="text-gray-600 fw-semibold" id="tbody">
                        ...
                        <tr id="tr_{{$row->paydrc_reference}}" class="text-start">
                            ...
                            <td class="text-center">
                                {!! Form::open(['method' => 'POST','route' => ['suppfin.success.single', $row->id],'style'=>'display:inline']) !!}
                                {!! Form::submit('S', ['class' => 'btn btn-success btn-border btn-sm rounded-2','data-toggle'=>'confirmation','data-placement'=>'left']) !!}
                                {!! Form::close() !!}

                                {!! Form::open(['method' => 'POST','route' => ['suppfin.failed.single', $row->id],'style'=>'display:inline']) !!}
    ...
    $(document).ready(function () {
        ...
        $('.failed-all').on('click', function(e) {
                ...
                // Start Multiple Paiement
                    ...
                    var redirect = "{{ url('suppfin/action-failed-all') }}";
                    $.ajax({
                        url: "{{ url('suppfin/action-failed-all') }}",
                        type: 'POST',
        ...
        $('.success-all').on('click', function(e) {
                ...
                // Start Multiple Paiement
                    ...
                    var strIds = idsArr.join(","); 
                    var redirect = "{{ url('suppfin/action-successfull-all') }}";
                    $.ajax({
                        url: "{{ url('suppfin/action-successfull-all') }}",
                        type: 'POST',
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        data: 'ids='+strIds,
                        processData: true,
        ...
        $('.delete-all').on('click', function(e) {
            ...
            {  
                ...
                if(confirm("Are you sure, you want to delete the selected transactions?")){  
                    var strIds = idsArr.join(","); 
                    $.ajax({
                        url: "{{ route('suppfin.transaction.delete.multiple') }}",
                        type: 'DELETE',
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        data: 'ids='+strIds,
                        processData: true,
