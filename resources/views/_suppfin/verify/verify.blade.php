                    ...
                    <tbody class="text-gray-600 fw-semibold" id="tbody">
                        ...
                        <tr id="tr_{{$row->switch_reference}}" class="text-start">
                            ...
                            {{-- <td class="text-center">
                                {!! Form::open(['method' => 'POST','route' => ['suppfin.success.single', Crypt::encrypt($row->switch_reference)],'style'=>'display:inline']) !!}
                                {!! Form::submit('S', ['class' => 'btn btn-success btn-border btn-sm rounded-2','data-toggle'=>'confirmation','data-placement'=>'left']) !!}
                                {!! Form::close() !!}

                                {!! Form::open(['method' => 'POST','route' => ['suppfin.failed.single', Crypt::encrypt($row->switch_reference)],'style'=>'display:inline']) !!}
    ...
    $(document).ready(function () {
        ...
        $('.verify-all').on('click', function(e) {
                ...
                // Start Multiple Paiement
                    ...
                    var redirect = "{{ url('suppfin/paydrc/transaction/verify') }}";
                    $.ajax({
                        url: "{{ url('suppfin/verify-select-ids') }}",
                        type: 'POST',
...
var myDropzone = new Dropzone(id, { // Make the whole body a dropzone
    url: "{{route('suppfin.transaction.verify.upload')}}", // Set the url for your upload script location
    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
    parallelUploads: 20,
